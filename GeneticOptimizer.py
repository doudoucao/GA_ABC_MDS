 ...
 *docs_representation
    a possible representation for the document to speed-up evaluation.
 *population_size
 ...
 *reproduction_rate
    percentage of summaries which will be bron in the new generation
 * maximization
...
class GeneticOptimizer(object):
    def __init__(self, fitness_fun, docs, docs_representation, max_length, population_size, survival_rate,
                 mutation_rate, reproduction_rate, maximization=False, sentences_rep=None):
        np.random.seed(123)
        ...
        for doc in docs:
            ...
            for s in doc:
                self._sentence_tokens.append(sentence_token(s.strip()))
    ...
    def _score_population(self, population):
        ...
        for individual in population:
            ...
            if self._sentences_rep is not None:
    ...
    def _len_individual(self, individual):
        ...
        for sentence in individual:
            len_ += len(sentence_token(sentence))
        return len_
    ...
    def _mutate(self, population, mutation_rate="auto"):
        ...
        for mutant in mutants:
            ...
            available_sentences = [s[0] for s in zip(self._sentences, self._sentence_tokens) if
                                   len(s[1]) <= available_size]
    ...
    def evolve(self, epoch):
        ...
        for i in xrange(epoch):
            ...
            y.append(best_individual[1])

            population = self._new_generation(scored_population)

        return best_individual, y
